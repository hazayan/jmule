...
public class PacketFactory {
    ...
    public static Packet getOfferFilePacket(ClientID userID) {
    	...
    	int sharedCount = SharingManagerFactory.getInstance().getFileCount();
    	ByteBuffer[] fileEntry = new ByteBuffer[sharedCount];
    	int enLen=0;
    	int i = 0;
    	for( SharedFile sFile : fileList){
    		...
    		fileEntry[i].put(sFile.getFileHash().getHash());
    		if (userID.isHighID()) {
    			fileEntry[i].put(userID.getClientID());
    			fileEntry[i].putShort((short)ConfigurationManagerFactory.getInstance().getTCP());
    		}
    		else { 
    			fileEntry[i].putInt(0); 
    			fileEntry[i].putShort((short)0); }
    		fileEntry[i].putInt(3);//Insert tag count
    		fileEntry[i].put(tagFileName.getData());
    		fileEntry[i].put(tagSize.getData());
    		fileEntry[i].put(tagType.getData());
    		i++;
    	}
    	...
    	packet.insertData(fileEntry[i].array());
    	}
    	return packet;
    }

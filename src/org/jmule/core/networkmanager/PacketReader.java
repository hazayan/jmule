...
public class PacketReader {
	...
	public static void readServerPacket(JMuleSocketChannel channel) throws JMEndOfStreamException, IOException, DataFormatException, UnknownPacketException {
		...
		if (packet_header != PROTO_EDONKEY_TCP) {
			throw new UnknownPacketException(packet_header, packet_opcode, packet_data.array());
		}
			...
			case PACKET_SRVMESSAGE : { 
				...
				case PACKET_SRVFOUNDSOURCES : {
					...
					List<Integer> port_list	= new LinkedList<Integer>();
				     ...
				     _network_manager.receivedSourcesFromServer(new FileHash(file_hash), clientid_list, port_list );
					 break;
					}
					...
					case PACKET_CALLBACKREQUESTED : {
						...
						_network_manager.receivedCallBackRequest(Convert.IPtoString(ip_address), port);
	...
	public static void readPeerPacket(JMuleSocketChannel channel) throws JMEndOfStreamException, IOException, DataFormatException, UnknownPacketException {
		ByteBuffer data2;
		...
		String peerIP = Convert.IPtoString(channel.getChannel().socket().getInetAddress().getAddress());
		...
		switch(packet_header) {
			...
			case  PROTO_EDONKEY_TCP : {
				...
				switch(packet_opcode) {
					...
					case OP_SENDINGPART : {
						...
						_network_manager.receivedRequestedFileChunkFromPeer(peerIP, peerPort, new FileHash(file_hash), new FileChunk(chunk_start, chunk_end, chunk_content));
						break;
					}
					...
					case OP_REQUESTPARTS : {
				    	...
				    	List<FileChunkRequest> chunks = new LinkedList<FileChunkRequest>();
				      	...
				      	_network_manager.receivedFileChunkRequestFromPeer(peerIP, peerPort, new FileHash(file_hash), chunks);
				      	
						break;
					}
					
					case OP_END_OF_DOWNLOAD : {
						_network_manager.receivedEndOfDownloadFromPeer(peerIP, peerPort);
						break;
					}
					...
					case OP_HASHSETREQUEST : {
						...
						_network_manager.receivedHashSetRequestFromPeer(peerIP, peerPort, new FileHash(file_hash));
						break;
					}
					...
					case OP_HASHSETANSWER : {
				    	...
				    	_network_manager.receivedHashSetResponseFromPeer(peerIP, peerPort, partSet);
						break;
					}
					...
					case OP_SLOTREQUEST : {
						byte[] file_hash = new byte[16];
						packet_data.get(file_hash);
						_network_manager.receivedSlotRequestFromPeer(peerIP, peerPort, new FileHash(file_hash));
						break;
					}
					...
					case OP_FILEREQUEST : {
						...
						_network_manager.receivedFileRequestFromPeer(peerIP, peerPort, new FileHash(file_hash));
						break;
					}
					...
					case OP_FILEREQANSWER : {
						...
						_network_manager.receivedFileRequestAnswerFromPeer(peerIP, peerPort, new FileHash(file_hash), new String(str_bytes.array()));
						break;
					}
					...
					case OP_FILEREQANSNOFILE : {
						...
						_network_manager.receivedFileNotFoundFromPeer(peerIP, peerPort, new FileHash(file_hash));
						break;
					}
					...
					case OP_FILESTATREQ : {
						...
						_network_manager.receivedFileStatusRequestFromPeer(peerIP, peerPort, new FileHash(file_hash));
					...
					case OP_FILESTATUS : {
				    	...
				    	_network_manager.receivedFileStatusResponseFromPeer(peerIP, peerPort, new FileHash(file_hash), bitSet);
						break;
					}
			...
			case  PROTO_EMULE_EXTENDED_TCP : {
				...
				case OP_EMULE_HELLO : {
					...
					TagList tag_list = readTagList(packet_data);
					_network_manager.receivedEMuleHelloFromPeer(peerIP, peerPort, client_version, protocol_version, tag_list);
					break;
				}
				...
				case OP_COMPRESSEDPART : {
			    	...
			    	_network_manager.receivedCompressedFileChunkFromPeer(peerIP, peerPort, new FileHash(file_hash), new FileChunk(chunkStart,chunkEnd,data));
			    	break;
				}
				
				case OP_EMULE_QUEUERANKING : {
					short queue_rank = packet_data.getShort();
					_network_manager.receivedQueueRankFromPeer(peerIP, peerPort, Convert.shortToInt(queue_rank));
					break;
				}
	...
	private enum PacketType { SERVER_UDP, PEER_UDP, KAD, KAD_COMPRESSED};
	public static void readUDPPacket(DatagramChannel channel) throws NetworkManagerException, UnknownPacketException {
		...
		try {
			...
			if (packet_content.get(0) == PROTO_EDONKEY_SERVER_UDP)
				...
				if (packet_content.get(0) == PROTO_EDONKEY_PEER_UDP)
					type = PacketType.PEER_UDP;
				else
				if (packet_content.get(0) == JKadConstants.PROTO_KAD_UDP) {
					if (!kad_enabled) return;
					type = PacketType.KAD;
				}
				else
				if (packet_content.get(0) == JKadConstants.PROTO_KAD_COMPRESSED_UDP) {
					if (!kad_enabled) return;
			...
			case PROTO_EDONKEY_SERVER_UDP : {
				switch ( packet_op_code )  {
					...
					case OP_SERVER_DESC_ANSWER : {
						...
						if (test_short < 0) new_packet = true;
						if (test_short > 500) new_packet = true;
					...
					default : {
						throw new UnknownPacketException(packet_protocol, packet_op_code, packet_content.array());
					}
				}
			}

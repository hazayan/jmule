...
import static org.jmule.core.jkad.JKadConstants.CONTACTS_SEND_HELLO;
...
import static org.jmule.core.jkad.JKadConstants.ROUTING_TABLE_CONTACT_ACCEP_TIME;
...
import static org.jmule.core.jkad.JKadConstants.ROUTING_TABLE_HELLO_SEND_INTERVAL;
...
public class RoutingTable {
	...
	private Task helloSender = null;
	private Task contact_checker = null;
	//private Task fakeLookup = null;
	...
	public void start() {
		...
		maintenanceTask = new Task() {
			...
			public void run() {
				...
				for(KadContact contact : maintenanceContacts ) {
					long contact_time = System.currentTimeMillis() - contact.getLastResponse();
					if (contact_time >= ROUTING_TABLE_CONTACT_TIMEOUT) {
						System.out.println("Remove contact \n" + contact);
						maintenanceContacts.remove(contact);
						removeNode(contact);
						continue;
					}
					
					if (contact_time <= ROUTING_TABLE_CONTACT_ACCEP_TIME) {
						maintenanceContacts.remove(contact);
				...
				for(int i = 0;i<ROUTING_TABLE_MAINTENANCE_CONTACTS; i++) {
					...
					}while(maintenanceContacts.contains(addContact));
					
					maintenanceContacts.add(addContact);
		...
		helloSender = new Task() {
			public void run() {
				List<KadContact> list = getRandomContacts(CONTACTS_SEND_HELLO);
				
				for(KadContact contact : list) {
					KadPacket hello_packet = PacketFactory.getHello2ReqPacket(TagList.EMPTY_TAG_LIST);
					JMUDPConnection.getInstance().sendPacket(hello_packet, contact.getIPAddress(), contact.getUDPPort());
				}
			}
		};
		Timer.getSingleton().addTask(ROUTING_TABLE_HELLO_SEND_INTERVAL, helloSender, true);
		...
		/*fakeLookup = new Task() {
			public void run() {
				Int128 clientID = new Int128(Utils.getRandomInt128());
				Int128 toleranceZone = new Int128();
				toleranceZone.setBit(127, true);
				if (find_node_task!=null)
					find_node_task.stopLookup();
				find_node_task = new LookupTask(RequestType.FIND_NODE, clientID, toleranceZone) {
					public void lookupTimeout() {
					}
				...
				Lookup.getSingleton().addLookupTask(find_node_task);
			}
		};
		Timer.getSingleton().addTask(ROUTING_TABLE_FAKE_LOOKUP_INTERVAL, fakeLookup, true);*/
	}

...
public class DownloadGeneralTab extends CTabItem implements Refreshable {
	...
	private Label part_status,download_status,sources_count, download_size,remaining_label,speed, eta;
	
	private GapListPainter gap_list_painter;
	private Canvas total_progress;

	public DownloadGeneralTab(CTabFolder tabFolder, final DownloadSession download_Session) {
		...
		transfer_group.setText(Localizer._("downloadinfowindow.tab.general.group.transfer"));
		...
		gap_list_painter = new GapListPainter(download_session.getGapList(),download_session.getFileSize());
		...
		sources_count.setText(download_session.getPeersCount()+"("+download_session.getPartialSources()+")");
		...
		label = new Label(transfer_group,SWT.NONE);
		label.setFont(skin.getLabelFont());
		label.setText(FileFormatter.formatFileSize(download_session.getFileSize()));
		...
		String download_eta = TimeFormatter.formatColon(download_session.getETA());
		...
		label.addListener(SWT.MouseUp, new Listener() {
			public void handleEvent(Event arg0) {
				Utils.setClipBoardText(download_session.getED2KLink().getAsString());
			}
		});

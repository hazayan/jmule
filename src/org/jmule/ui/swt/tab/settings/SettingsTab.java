...
public class SettingsTab extends AbstractTab {
	...
	private List<AbstractSettingsTreeItem> tab_list = new ArrayList<AbstractSettingsTreeItem>();
	private Tree tabs_tree;
	private AbstractSettingsTreeItem selectedTab = null;
	private TreeItem selectedItem   = null;
	
	public SettingsTab(Composite parent) {
		...
		tab_content.setLayout(new GridLayout(2,false));
		tab_content.setLayoutData(new GridData(GridData.FILL_BOTH));

		tabs_tree = new Tree(tab_content, SWT.BORDER);
		
		GridData ld = new GridData(GridData.FILL_VERTICAL);
		ld.widthHint = 120;
		tabs_tree.setLayoutData(ld);
		
		Composite content = new Composite(tab_content, SWT.NONE);
		content.setLayout(new GridLayout(1,false));
		content.setLayoutData(new GridData(GridData.FILL_BOTH));
		
		settings_tab_panel = new ScrolledComposite(content,SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL);
		settings_tab_panel.setLayoutData(new GridData(GridData.FILL_BOTH));
		settings_tab_panel.setAlwaysShowScrollBars(false);
		...
		//tab_list.add(new ProxyTab(settings_tab_panel));
		...
		tabs_tree.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				AbstractSettingsTreeItem selected_tab = (AbstractSettingsTreeItem) item.getData(DATA_KEY);
				if (selectedTab != null) {
					selectedTab.reset();
				...
				settings_tab_panel.setMinSize(selected_tab.getTabContent().computeSize(SWT.DEFAULT, SWT.DEFAULT));
			}
			
		});
		
		Composite button_bar = new Composite(content,SWT.NONE);
		...
		if (default_select_item != null) {
			...
			AbstractSettingsTreeItem selected_tab = (AbstractSettingsTreeItem) default_select_item.getData(DATA_KEY);
			selectedTab = selected_tab;
			settings_tab_panel.setContent(selected_tab.getTabContent());
			settings_tab_panel.setMinSize(selected_tab.getTabContent().computeSize(SWT.DEFAULT, SWT.DEFAULT));
		}
		...
		/*if (JMConstants.isOSX) {
			...
			if (default_select_item != null) {
				tabs_tree.select(default_select_item);
				AbstractSettingsTab selected_tab = (AbstractSettingsTab) default_select_item.getData(DATA_KEY);
				selectedTab = selected_tab;
				settings_tab_panel.setContent(selected_tab.getTabContent());
			}
		}*/
	...
	private void apply(List<AbstractSettingsTreeItem> tabList) {
		if (tabList == null) return;
		for(AbstractSettingsTreeItem tab : tabList) {
			tab.save();
			apply(tab.getChildTabs());
		}
	}
	
	private void completeList(TreeItem parent, List<AbstractSettingsTreeItem> tabList) {
		if (tabList == null) return;
		TreeItem item;
		for(AbstractSettingsTreeItem tab : tabList) {
			if (parent == null)
